...
const archiveIds = ["5c7201e1-0030-43aa-86a9-3bc65c683701", "ecdc7552-755c-4783-b3d8-cc1d1a6e4669", "a254738d-712f-44d9-b61c-8b09144d3cc5", "b4fad011-fa28-4cca-8b1d-7bd85a557868", "14640c00-4329-4d3d-84e3-97dc886a24cd"];
...
for (const id of [...recoverIds, ...archiveIds]) {
  ...
  if (recoverIds.includes(id)) {
    ...
    if (!["failed", "provider_timeout"].includes(job.status) || job.result_url || job.storage_path) throw new Error(`Failure archive precondition changed: ${id}`);
    if (apply) {
      const updated = await json(`${table}?${filter}&status=eq.${job.status}`, {
        ...
        body: JSON.stringify({ completed_at: job.completed_at || job.updated_at || job.created_at,
          result_metadata: { ...job.result_metadata, archivedAt: new Date().toISOString(), archiveReason: "coze-verification-failed-test-audio", originalStatus: job.status } }),
      });
