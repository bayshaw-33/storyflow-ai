...
test("development chat follows the interface language", () => {
  const zh = buildPrompt({ taskType: "creation_development_chat", input: "吸血鬼爱情", options: { interfaceLanguage: "zh-CN" } });
  const en = buildPrompt({ taskType: "creation_development_chat", input: "vampire romance", options: { interfaceLanguage: "en-US" } });
...
test("novel and screenplay unit prompts request distinct output structures", () => {
  const novel = buildPrompt({
    taskType: "creation_novel_unit",
    input: "写第一章",
    options: { contentMode: "novel", sourceLanguage: "English", unitNo: 1, generationScope: "unit" },
  });
  const screenplay = buildPrompt({
...
test("translation remains optional and localization returns paired audit sections", () => {
  const translation = buildPrompt({
    taskType: "creation_translate_unit",
    input: "source",
    options: { sourceLanguage: "English", translationLanguage: "Chinese", unitNo: 2 },
  });
  const localization = buildPrompt({
