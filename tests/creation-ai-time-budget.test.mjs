...
test("角色圣经主模型与备用模型共用前端 240 秒以内的总预算", async () => {
  ...
  try {
    ...
    assert.ok(timeoutBudgets[0] >= 120_000, "主模型至少应获得 120 秒");
    assert.ok(timeoutBudgets[1] >= 60_000, "备用模型至少应获得 60 秒");
    assert.ok(
      timeoutBudgets[0] + timeoutBudgets[1] <= 210_000,
      `Provider 总预算必须给前端和服务端收尾留余量，实际为 ${timeoutBudgets.join(" + ")}`,
    );
  } finally {
