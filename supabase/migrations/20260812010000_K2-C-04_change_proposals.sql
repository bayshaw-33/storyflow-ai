...
BEGIN
  ...
  IF jsonb_array_length(COALESCE(p_items, '[]'::jsonb)) = 0 THEN RAISE EXCEPTION 'CHANGE_PROPOSAL_EMPTY_ITEMS'; END IF;
    ...
    FOR v_item IN SELECT * FROM public.storyflow_change_proposal_items WHERE proposal_id = v_proposal.id ORDER BY id FOR UPDATE LOOP
      ...
      ELSIF v_item.object_type IN ('relationship', 'timeline_event') THEN
        IF v_item.object_type = 'relationship' THEN
          IF v_item.object_id IS NULL THEN INSERT INTO public.storyflow_universe_relationships (universe_id, user_id, source_entity_id, target_entity_id, relationship_type, relationship_status, summary, status, source_project_id) VALUES (p_universe_id, p_user_id, NULLIF(v_payload->>'sourceEntityId', '')::uuid, NULLIF(v_payload->>'targetEntityId', '')::uuid, COALESCE(v_payload->>'relationshipType', 'related'), COALESCE(v_payload->>'relationshipStatus', 'active'), COALESCE(v_payload->>'summary', ''), 'canon', v_proposal.source_project_id) RETURNING id INTO v_item.object_id;
          ELSE UPDATE public.storyflow_universe_relationships SET source_entity_id = COALESCE(NULLIF(v_payload->>'sourceEntityId', '')::uuid, source_entity_id), target_entity_id = COALESCE(NULLIF(v_payload->>'targetEntityId', '')::uuid, target_entity_id), relationship_type = COALESCE(v_payload->>'relationshipType', relationship_type), relationship_status = COALESCE(v_payload->>'relationshipStatus', relationship_status), summary = COALESCE(v_payload->>'summary', summary), updated_at = now() WHERE id::text = v_item.object_id AND universe_id = p_universe_id; IF NOT FOUND THEN RAISE EXCEPTION 'CHANGE_PROPOSAL_TARGET_NOT_FOUND'; END IF; END IF;
        ELSE
          IF v_item.object_id IS NULL THEN INSERT INTO public.storyflow_universe_timeline_events (universe_id, user_id, title, description, date_label, season_number, episode_number, order_index, related_entity_ids, is_canon, status, source_project_id) VALUES (p_universe_id, p_user_id, COALESCE(v_payload->>'title', ''), COALESCE(v_payload->>'description', ''), COALESCE(v_payload->>'dateLabel', ''), (v_payload->>'seasonNumber')::integer, (v_payload->>'episodeNumber')::integer, (v_payload->>'orderIndex')::integer, COALESCE(v_payload->'relatedEntityIds', '[]'::jsonb), true, 'canon', v_proposal.source_project_id) RETURNING id INTO v_item.object_id;
          ELSE UPDATE public.storyflow_universe_timeline_events SET title = COALESCE(v_payload->>'title', title), description = COALESCE(v_payload->>'description', description), date_label = COALESCE(v_payload->>'dateLabel', date_label), season_number = COALESCE((v_payload->>'seasonNumber')::integer, season_number), episode_number = COALESCE((v_payload->>'episodeNumber')::integer, episode_number), order_index = COALESCE((v_payload->>'orderIndex')::integer, order_index), related_entity_ids = COALESCE(v_payload->'relatedEntityIds', related_entity_ids), updated_at = now() WHERE id::text = v_item.object_id AND universe_id = p_universe_id; IF NOT FOUND THEN RAISE EXCEPTION 'CHANGE_PROPOSAL_TARGET_NOT_FOUND'; END IF; END IF;
        END IF;
