...
function CastingContent() {
  ...
  const [state, setState] = useState<ProductionProjectState>(() =>
    createEmptyProductionState({ projectId, mode: "casting" }),
  );
  const [loadError, setLoadError] = useState<string | null>(null);
  const sync = useProductionSync(session, projectId);

  useEffect(() => {
    document.title = "选角配置 | Kiikis";
  }, []);
  ...
  const handleCastingChange = useCallback(
    (characterId: string, actorId: string | null) => {
      setState((current) => {
        const nextCasting = { ...current.casting };
        if (actorId) {
          nextCasting[characterId] = actorId;
        } else {
          delete nextCasting[characterId];
        }
        const next: ProductionProjectState = {
          ...current,
          casting: nextCasting,
          updatedAt: new Date().toISOString(),
        };
        if (session) {
          void sync.saveToCloud(next).catch(() => {});
        }
        return next;
      });
    },
