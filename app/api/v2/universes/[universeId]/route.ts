...
import { readUniverse, V2UniverseError } from "@/lib/server/v2/universe";
...
export async function GET(request: NextRequest, context: { params: Promise<{ universeId: string }> }) {
  try {
    ...
    const result = await readUniverse({ fetcher: serviceFetch, userId: user.id, universeId });
    ...
    return universeErrorResponse(error, "Unable to read the universe.");
  }
}
