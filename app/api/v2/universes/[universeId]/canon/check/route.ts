...
import { CanonError, runCanonCheck } from "@/lib/server/v2/canon";
...
export async function POST(request: NextRequest, context: { params: Promise<{ universeId: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, contractVersion: "2.0.0-alpha.1", ...(await runCanonCheck({ fetcher: serviceFetch, userId: user.id, universeId, input: body })) });
  } catch (error) {
