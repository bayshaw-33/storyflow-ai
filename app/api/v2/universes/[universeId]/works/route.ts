...
import { readUniverseWorks, V2UniverseError } from "@/lib/server/v2/universe";
...
export async function GET(request: NextRequest, context: { params: Promise<{ universeId: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, contractVersion: "2.0.0-alpha.1", ...(await readUniverseWorks({ fetcher: serviceFetch, userId: user.id, universeId })) });
  } catch (error) {
    ...
    return universeErrorResponse(error, "Unable to read universe works.");
  }
}
