...
import { getSellerListings } from "@/lib/supabase/marketplace-queries";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * GET /api/dashboard/sales/listings?cursor=0&limit=12
 * 我的上架（含销量统计，必须登录）。
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getSellerListings(client, user.id, cursor, limit);
    ...
    return NextResponse.json({ success: false, error: "读取上架列表失败。" }, { status: 500 });
  }
}
