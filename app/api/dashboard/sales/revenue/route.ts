...
import { getRevenueLedger } from "@/lib/supabase/marketplace-queries";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * GET /api/dashboard/sales/revenue?cursor=0&limit=12
 * 收益明细（必须登录）。
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await getRevenueLedger(client, user.id, cursor, limit);
    ...
    return NextResponse.json({ success: false, error: "读取收益明细失败。" }, { status: 500 });
  }
}
