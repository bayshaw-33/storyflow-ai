...
const promptByTask: Record<TaskType, string> = {
  ...
  creation_plot_outline: [
    ...
    "先输出“---STORY_SPINE---”，写全局主线、关键因果、核心反转与终局，只保留骨架级信息。",
    "再输出“---PLOT_OUTLINE---”，展开为可解析 Markdown 结构，并使用“## 大章 N｜标题”和“### 第 N 章/集｜标题”。",
    "大章数量和每章/集数量按用户确认内容决定，不得强制固定规模。整体输出要分段清晰、便于前端保存。",
  ].join("\n"),
  ...
  creation_localize_unit: [
    ...
    "格式强制要求（违反将导致整段输出被丢弃）：",
    "1. 三个标记必须独占一行，前后不得有其他字符（不得用 markdown 标题/加粗包裹）。",
    "2. 标记格式严格为 ---KEY---（三个减号 + 大写KEY + 三个减号），不得写成 ### KEY 或 **KEY** 或 --- KEY ---。",
    "3. 每一段内容必须紧跟在对应标记之后，不得缺少任何一段。",
  ].join("\n"),
